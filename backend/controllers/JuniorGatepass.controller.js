...
const createJuniorGatepass = async (req, res) => {
    try {
        const {
            student_id,
            teacher_id,
        ...
        const { data: student, error: studentError } = await supabase
            ...
            .select("id, name, registration_number, department, class_name, section")
            .eq("id", student_id)
            .single();
        ...
        if (studentError || !student) {
            return res.status(404).json({
                success: false,
                message: "Junior student not found.",
            });
        }
        ...
        if (teacher.teaching_level !== "JUNIOR") {
            return res.status(400).json({
            success: false,
            message: "Selected teacher is not a junior teacher.",
            });
        }
        ...
        const filePath = `junior/${student_id}/${fileName}`;
        ...
        if (gatepassError) {
            ...
            return res.status(500).json({
                success: false,
                message: "Failed to create junior gatepass.",
            });
        }
        ...
        await createAuditLog({
            ...
            entity_type: "JUNIOR_GATEPASS",
            entity_id: gatepass.id,
            description: `Junior gatepass created for ${student.name}.`
        });

        const { error: eventError } = await supabase.from("junior_gatepass_events").insert({
            gatepass_id: gatepass.id,
            event_type: "CREATED",
            event_time: gatepass.created_at || new Date().toISOString(),
            notes: "Junior gatepass created."
        });
        if (eventError) console.error("Create junior gatepass event error:", eventError);

        return res.status(201).json({
            success: true,
            message: "Junior gatepass created successfully.",
            gatepass,
        });
...
const searchJuniorGatepasses = async (req, res) => {
    try {
        ...
        if (!students || students.length === 0) {
            return res.status(404).json({
                success: false,
                message: "No junior student found.",
            });
        }

        const studentIds = students.map(student => student.id);

        // Find gatepasses belonging to those students
...
const markJuniorGatepassExited = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await createAuditLog({
            actor_type: "SECURITY",
            action: "EXIT_JUNIOR_GATEPASS",
            entity_type: "JUNIOR_GATEPASS",
            entity_id: gatepass.id,
            description: `Junior student ${gatepass.student_id} exited using gatepass.`
        });

        // Create event
        const { error: eventError } = await supabase
            .from("junior_gatepass_events")
            .insert({
                gatepass_id: id,
                event_type: "EXITED",
...
const getJuniorStudentForGatepass = async (req, res) => {
    try {
